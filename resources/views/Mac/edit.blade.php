    ...
    <div class="row">
        <div class='col-md-12'>
            <div class="box box-primary">
                ...
                @endif
                <form role="form" action="{{ route('macs.update', $mac->id) }}" method="post">
                    <div class="box-header with-border">
                        <h3 class="box-title">Editar Mac</h3>
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control" id="dispositivo" name="id_dev">
                                    @foreach ($dev as $d)
                                        <option value="{{ $d->id }}" {{ $d->id == $mac->id_dev ? 'selected' : '' }}>{{ $d->descricao }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="form-group">
                                <label for="nome">Ticket</label>
                                <input type="text" class="form-control" id="ticket" placeholder="Ticket OTRS" name="ticket" value="{{ $mac->ticket }}">
                            </div>
                            <div class="form-group">
                                <label for="expira">Expira</label>
                                <input type="date" class="form-control" id="expira" name="expira" value="{{ $mac->expira->format('Y-m-d') }}">
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="form-group">
                                <label for="searchname">Usuário</label>
                                <input type="text" name="searchname" class="form-control" id="searchname" placeholder="Nome do usuário" value="{{ $mac->usuarios->nome }}">
                                <input type="hidden" name="id_user" class="form-control" id="id_user" value="{{ $mac->id_user }}">
                            </div>
                            <div class="form-group">
                                <label for="descricao">Descrição</label>
                                <input type="text" class="form-control" id="descricao" placeholder="Descricao" name="nome_eq" value="{{ $mac->dispositivo->descricao }}">
                            </div>
                            <div class="form-group">
                                <label for="sei">SEI</label>
                                <input type="text" class="form-control" id="sei" placeholder="SEI" name="sei" value="{{ $mac->sei }}">
                            </div>
                            <div class="form-group">
                                <label for="ativo">Ativo</label><br/>
                                <input type="checkbox" name="ativo" value="1" data-toggle="toggle" data-on="Sim" data-off="Não" data-onstyle="success" data-offstyle="danger" {{ $mac->ativo == 1 ? 'checked' : '' }}>
                            </div>
                            </div>
                        </div>
                    </div><!-- /.box-body -->
